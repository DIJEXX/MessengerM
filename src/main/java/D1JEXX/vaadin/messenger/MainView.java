package D1JEXX.vaadin.messenger;
...
public class MainView extends VerticalLayout {
    ...
    private void buildChat() {
        ...
        chat.add(
                new H3("🤢 Wazzup messenger"),
                grid,
                new HorizontalLayout() {{
                    add(
                            field,
                            new Button("Напишите сообщение...") {{
                                addClickListener(click -> {
    ...
    private String renderRow(Storage.ChatMessage message) {
        if (message.getName().isEmpty()) {
            return Processor.process(String.format("**%s** присоединился к чату!", message.getMessage()));
        } else {
